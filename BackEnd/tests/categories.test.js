...
let idNotFound='a';
  ...
  it('create category, with admin credentials, fails when name already exists in database', async function () {
    ...
    .post('/api/categories')
    .set("Authorization", `Bearer ${adminToken}`)
    .send({name: "Category demo UNO", description: "description Test", image: "https://Test.com/600/92c952" })
    expect(response.status).to.eql(403)
    expect(response.body).to.be.an('object')
  });
  ...
  // GET ALL CATEGORIES
  it('list all categories whithout pagination, success with any credentials', async function () {
    const response = await request
    .get('/api/categories') 
    .set("Authorization", `Bearer ${regularToken}`)
  ...
  it('list one category, with admin credentials, fails when id not found', async function () {
    ...
    .set("Authorization", `Bearer ${adminToken}`)
    .send({name: "Category demo MIL", description: "demo accusamus beatae ad facilis cum similique qui sunt", image: "https://via.placeholder.com/600/92c952" })       
  ...
  it('update category, with admin credentials, fails when name already exists in database', async function () {
    ...
    .put(`/api/categories/${idTest}`)   
    .set("Authorization", `Bearer ${adminToken}`)
    .send({name: "Category demo SEIS", description: "description Test", image: "https://Test.com/600/92c952" })
    expect(response.status).to.eql(403)
    expect(response.body).to.be.an('object')
  });
